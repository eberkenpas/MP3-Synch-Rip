...
### Scripts
- `yt-audio.sh` - YouTube download script. Usage: `./yt-audio.sh '<youtube-url>'`
...
### Documentation
- `readme.md` - Manual yt-dlp command reference
- `bashinstructions.md` - Quick usage guide for the shell script
...
### Future Ideas
- [ ] Add YouTube playlist support to yt-audio.sh
...
- [ ] Add delete/cleanup option to sync script
- [ ] Add backup script to archive current Music folder
- [ ] Add audiobook splitting for large files (FAT32 4GB limit)
  - Split oversized audiobooks using ffmpeg stream copy
  - Generate M3U playlists for split parts
  - Integrate with sync workflow to auto-split before transfer
- [ ] Add progress bar to sync tool for file transfers

## Next Steps
- [ ] Configure downloads to target media server at `192.168.50.182:5000`
- [ ] Modify `yt-audio.sh` to optionally download directly to server
